"""Unit tests for the sonnenbackup integration init."""
...
async def test_async_setup_entry_success(
    ...
    with (
        patch("custom_components.sonnenbackup.IFSEI", return_value=mock_ifsei),
        patch("custom_components.sonnenbackup.Path") as mock_path,
        patch(
...
async def test_async_setup_entry_vol_error(
    ...
    with patch("custom_components.sonnenbackup.IFSEI", return_value=mock_ifsei):
        await hass.config_entries.async_setup(mock_config_entry.entry_id)
        await hass.async_block_till_done()
...
@pytest.mark.asyncio
async def test_sonnenbackup_updatable_entity_available() -> None:
    ...
    mock_ifsei.name = "sonnenbackup IFSEI"
    mock_ifsei.get_device_id.return_value = "ifsei_device_id"

    entity = sonnenbackupUpdatableEntity(device, mock_ifsei)
...
@pytest.mark.asyncio
async def test_sonnenbackup_updatable_entity_device_info() -> None:
    ...
    assert device_info.get("identifiers") == {("sonnenbackup", "test_device_unique_id")}
    assert device_info.get("via_device") == ("sonnenbackup", "test_device_unique_id")
